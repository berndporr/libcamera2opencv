...
class FormatConverter
{
  public:
     ...
     * Note that libcamera::formats::RGB888 actually is BGR (!) so that it's
     * compatible with openCV's BGR and thus won't need conversion.
     */
    static constexpr libcamera::PixelFormat nativeInputFormat
        = libcamera::formats::RGB888;

    /**
     * The output openCV format no matter what libcamera's native input format
     * is. We always output 8 bit unsigned BGR, the native openCV format.
     */
     ...
     */
    void start (const libcamera::PixelFormat format, int width, int height,
                int stride);
    ...
    /**
     * Destroy the Format Converter object. This also frees up any internal
     * memory.
     */
  ...
  private:
    ...
    void yuv_to_rgb (const int y, const int u, const int v, int *r, int *g,
                     int *b) const;
    ...
    /**
     * Converts from to BGR from various RGB/RGBA/BGRA formats. Remember that
     * libcamera's BGR is actually RGB (!) so needs to be re-shuffled.
     *
    ...
    cv::Mat
    convertYUVPacked (const std::vector<libcamera::Span<uint8_t> > &planes);
    cv::Mat
    convertYUVPlanar (const std::vector<libcamera::Span<uint8_t> > &planes);
    cv::Mat convertYUVSemiPlanar (
