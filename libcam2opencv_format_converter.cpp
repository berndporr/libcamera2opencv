...
void FormatConverter::start (const libcamera::PixelFormat format, int width,
    ...
    if (nativeInputFormat == format)
        {
            ...
            // Nothing to do here further as we can directly return a cv::Mat
            // pointing to the framebuffer!
            return;
        }
        ...
        case libcamera::formats::MJPEG:
            ...
            if (!tjInstance)
                throw std::runtime_error (
                    "Failed to initialize TurboJPEG decompressor.");
            break;
        default:
...
cv::Mat
FormatConverter::convert (const std::vector<libcamera::Span<uint8_t> > &planes)
{
    switch (formatFamily_)
        {
        case NATIVE:
            return cv::Mat (height_, width_,
                            FormatConverter::openCVoutputFormat,
                            planes[0].data (), stride_);
        case MJPEG:
...
void FormatConverter::yuv_to_rgb (const int y, const int u, const int v,
...
cv::Mat FormatConverter::convertJPG (
    const std::vector<libcamera::Span<uint8_t> > &planes)
{
    ...
    if (tjDecompressHeader3 (tjInstance, planes[0].data (), planes[0].size (),
        ...
        != 0)
        {
            std::cerr << "tjDecompressHeader3 error: " << tjGetErrorStr ()
                      << std::endl;
        }
    ...
    if ((w == (int)width_) && (h == (int)height_))
        {
            // Decompress to BGR
            if (tjDecompress2 (tjInstance, planes[0].data (),
                               planes[0].size (), dstImage.data, w, 0, h,
                               TJPF_BGR, TJFLAG_FASTDCT)
                != 0)
                {
                    const std::string errstr = tjGetErrorStr ();
                    // print only error if it's not the warning about
                    // extraneous bytes
                    if (errstr.find ("extraneous bytes before marker")
                        == std::string::npos)
                        std::cerr << "tjDecompress2 error: " << errstr
                                  << std::endl;
                }
        }
    else
        {
            fprintf (stderr,
                     "Wrong image size. Can't decompress. Have= %d x %d, "
                     "Want= %d x %d.\n",
                     width_, height_, w, h);
        }
    return dstImage;
...
cv::Mat FormatConverter::convertRGB (
    const std::vector<libcamera::Span<uint8_t> > &planes)
{
...
cv::Mat FormatConverter::convertYUVPacked (
    const std::vector<libcamera::Span<uint8_t> > &planes)
{
    const unsigned char *src = planes[0].data ();
...
cv::Mat FormatConverter::convertYUVPlanar (
    const std::vector<libcamera::Span<uint8_t> > &planes)
{
    ...
    for (unsigned int y = 0; y < height_; y++)
        {
            ...
            const unsigned char *line_cb
                = src_cb + (y / vertSubSample_) * c_stride;
            const unsigned char *line_cr
                = src_cr + (y / vertSubSample_) * c_stride;
...
cv::Mat FormatConverter::convertYUVSemiPlanar (
    const std::vector<libcamera::Span<uint8_t> > &planes)
{
    ...
    for (unsigned int y = 0; y < height_; y++)
        {
            ...
            const unsigned char *src_cb
                = src_c + (y / vertSubSample_) * c_stride + cb_pos;
            const unsigned char *src_cr
                = src_c + (y / vertSubSample_) * c_stride + cr_pos;
